...
class MicrowaveRAG:
    ...
    def retrieve_context(self, query: str, k: int = 4, score=0.3) -> str:
        ...
        relevant_docs = self.vectorstore.similarity_search_with_relevance_scores(query=query, k=k, score_threshold=score)
        context_parts = []
...
def main(rag: MicrowaveRAG):
    ...
    while True: 
        user_question = input("\n> ").strip()
        # Step 1: Retrieval
        context = rag.retrieve_context(user_question, k=4, score=0.0)
        # Step 2: Augmentation
...
main(
    MicrowaveRAG(AzureOpenAIEmbeddings(deployment='text-embedding-3-small-1', azure_endpoint=DIAL_URL, api_key=SecretStr(API_KEY)), AzureChatOpenAI(temperature=0.0,azure_deployment='gpt-4o', azure_endpoint=DIAL_URL, api_key=SecretStr(API_KEY), api_version=""))
)